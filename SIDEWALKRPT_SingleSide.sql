IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[SIDEWALKRPT_SingleSide]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[SIDEWALKRPT_SingleSide]
GO
...
-- SingleSided report.
--
-- If @IsFiltered=1, assumes that the #Filter table
--  containing filtered Sidewalk IDs has been built.
--
-- =============================================
CREATE PROCEDURE [dbo].[SIDEWALKRPT_SingleSide]
	@SidewalkID integer
...
BEGIN
	SET NOCOUNT ON;
		...
		WHERE O.SidewalkID = @SidewalkID

		-- Inspection
		SELECT ISNULL(ic.[Description], 'Not Rated') as Condition, i.InspectDate, ISNULL(i.Inspector,'') as Inspector, 
			   ISNULL(i.Memo,'') as InspectionMemo, ia.Memo as iActivityMemo, ISNULL(iAct.[Description],'') as iActivityType
		...
		SELECT w.UserWorkOrderID as woID, ws.[Description] as woStatus, wa.[Description] as woActivity, 
				...
				w.EnteredDate as woEnteredDate, w.[Description] as woDetails, w.WorkerComments as woComments
